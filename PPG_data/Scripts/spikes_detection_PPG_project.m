function [spikes,fwhm] = spikes_detection_PPG_project(signals,fs,FWHM_thr);

if nargin<3
    FWHM_thr = inf; % ms
end

sensthr_tot = [0.3 0.2 0.1];
...
for ithr = 1:length(sensthr_tot)
    ...
    if ~isempty(iiok)
        ...
        W_tot{ithr} = W{iiSD};
        A = abs(signals(round(spikes/1000*fs)));
        iiko = A<median(A,'omitmissing')/4; % Remove annotation on isoelectric line
        spikes_tot{ithr} = spikes(~iiko);
